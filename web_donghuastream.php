...
$data = [];
$url  = $_ENV['URL_DONGHUASTREAM'] ?? 'https://donghuastream.org';
...
$crawler->input($url)
    ->addStep(Http::get())
    ->addStep(
        Html::each('.releases.latesthome ~ .listupd.normal > .excstf > article.bs')
            ->extract([
    ...
    try {
        if (! empty($data['title']) && ! empty($data['episode']) && ! empty($data['link'])) {
            ...
            if (! empty($foundRow) && $write) {
                $updateRange = 'Donghua!V' . $foundRow . ':X' . $foundRow;
